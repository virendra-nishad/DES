...
void set_the_key(int sw1,BYTE *CD,int r)
{
  ...
  { 
	...
	//divide key into 2 parts of 28 bits each
	//left shift both parts as specified by DES algo. EG: in case of round 1, left shift by 1, in case of round 2, left shift by 1,
	//in case of round 3, left shift by 2 and so on...
	...
	//i.e. order varies in case of encryption and decryption
	j = sw1 ? r-1-i :i ;   /**--*/

	/* Permute C and D with PC2 to generate KS[i] */
	for (k=0; k< 48 ; k++)  
	{
		//since, values in PC2 range from 1 onwards, therefore, subtracting 1.	
		KS[j][k] = CD[PC2[k] -1];
		//cout<<"key for round: "<< i <<" : " <<KS[j][k]<<endl;
		
		printf("key for round %d: %x\n", i , KS[j][k]);
	}
...
int z;
  unpack8(in, block);
	...
	{  
		...
		for( j=0; j<48; j++) 
		{
			//Todo: cross verify how expasion is happening
...
int main()
{
    ...
    ifile1.open("binary256.txt");

    while( getline(ifile1,str1))
    {
	    cout<<"Trying key: " << str1 << "\n";

	    //convert key from ascii to numerical form
	    for(i=0;i<56;i++)
	    	key[i]=str1[i]-48;
	    ...
	    sw1=0;	// type of operation 0= encryption, 1= decryption 
	    r=3;	//round

		//plain and cipher text in byte form
	    BYTE plain[9]={101,5,210,32,247,58,88,112,'\0'};
	    BYTE cipher1[9]={164,25,193,155,238,35,156,20,'\0'};

	    flag='N';
	    h=0;


		//for each round generate 48 bit keys from 56 bit key 
	    set_the_key(sw1,key,r);

		//pass plain text, buffer to store the calculated cipher text, number of rounds
	    ...
	    for(i=0;i<8 ;i++)
	    {
		printf("cipher1[%d]: %x, cipher2[%d]: %x\n", i, cipher1[i], i,  cipher2[i]);	
		if(int(cipher1[i])!=int(cipher2[i]))
		    h=1;
	    }

		//h==0 tells key successfuly found
	    if(h==0)
	    {
		for(i=0;i<56;i++)
		{
